...
class RenewalScheduler:
    ...
    def start(self):
        ...
        if not self.running:
            ...
            self.logger.info(f"Renewal scheduler started with {self.check_interval_hours}h interval")
        ...
        try:
            if not renewal_results:
                ...
                crud_log.create_log(
                    db=db,
                    level="INFO",
                    message="Automatic renewal check completed - no certificates due for renewal",
                    source="RenewalScheduler"
                )
            else:
                ...
                summary_message = f"Automatic renewal check completed: {len(successful_renewals)} successful, {len(failed_renewals)} failed"
                ...
                for result in renewal_results:
                    ...
                    if result.get('success', False):
                        message = f"Certificate '{cert_name}' (deployment {deployment_id}) renewed and deployed successfully"
                        crud_log.create_log(
                            db=db,
                            level="INFO", 
                            message=message,
                        ...
                        message = f"Certificate '{cert_name}' (deployment {deployment_id}) renewal failed in {phase}: {error}"
                        crud_log.create_log(
                            db=db,
                            level="ERROR",
                            message=message,
