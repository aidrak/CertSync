...
class DigitalOceanDns(DnsProviderBase):
    ...
    def _get_domain_and_subdomain(self, full_domain: str):
        ...
        # A simple heuristic: find the longest suffix that is a registered domain in the account.
        # This is complex. A simpler, common approach is to assume the last two parts are the domain.
        # This works for "example.com" but not "example.co.uk".
        # For this implementation, we'll assume the user provides the root domain.
        # A better implementation would get all domains from the account and find the matching one.
        # Let's assume the challenge domain is always `_acme-challenge.subdomain.domain.com`
    ...
    def delete_txt_record(self, domain: str, token: str):
        ...
        try:
            ...
            # Don't raise an exception here, as failure to clean up shouldn't block the whole process.
