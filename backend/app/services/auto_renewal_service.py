...
class AutoRenewalService:
    ...
    async def check_and_renew_due_certificates(self) -> List[dict]:
        ...
        try:
            ...
            now = datetime.utcnow()
            due_deployments = db.query(Deployment).filter(
                Deployment.auto_renewal_enabled == True,
    ...
    async def renew_certificate_and_deploy(self, db: Session, deployment: Deployment) -> dict:
        ...
        self.logger.info(f"Starting renewal for deployment {deployment.id}: {certificate.common_name} -> {target_system.system_name}")
        ...
        try:
            ...
            self.logger.error(f"Deployment failed for {certificate.common_name} to {target_system.system_name}: {e}")
            return {
                'deployment_id': deployment.id,
                'certificate_name': certificate.common_name,
                'success': False,
    ...
    async def renew_certificate_cloudflare(self, db: Session, certificate: Certificate) -> dict:
        ...
        try:
            ...
            if not dns_account or dns_account.provider_type != 'cloudflare':
                raise Exception(f"Certificate {certificate.common_name} is not using Cloudflare DNS")
    ...
    async def deploy_to_target_system(self, db: Session, deployment: Deployment) -> dict:
        ...
        """
        target_system = deployment.target_system
        self.logger.info(f"Phase 2: Deploying certificate to {target_system.system_type.value} system: {target_system.system_name}")
        ...
        try:
            ...
            if target_system.system_type.value == "sonicwall":
                ...
                # Check for success
                deployment_success = any("SUCCESS" in log or "successful" in log.lower() for log in deployment_logs[-3:])
                ...
                if deployment_success:
                    ...
                    db.query(models.Deployment).filter(models.Deployment.id == deployment.id).update({
                        "last_deployed_at": datetime.utcnow()
                    })
                    db.commit()
                    
                    self.logger.info(f"Automatic deployment successful for {certificate.common_name}")
                    return {
                        'success': True,
                        'logs': deployment_logs,
                        'message': f"Certificate automatically deployed to {target_system.system_name}"
                    }
                else:
                    crud_deployment.update_deployment_status(
                        db, 
                ...
                raise Exception(f"Automatic deployment not yet implemented for {target_system.system_type.value}")
