...
class SonicWallValidator:
    def __init__(self, firewall_settings):
        ...
        self.base_url = f"https://{self.firewall_settings.public_ip}:{self.firewall_settings.port}/api/sonicos"
    ...
    async def _make_request(self, session, method: str, endpoint: str, data=None, timeout: int = 30):
        ...
        try:
            ...
            async with response_obj as response:
                ...
                try:
                    if raw_text.strip().startswith(("{", "[")) and "application/json" in response.headers.get("Content-Type", "").lower():
    ...
    async def check_certificate_exists(self, session, cert_name: str, context: str = "") -> bool:
    ...
    def _generate_test_certificate(self, common_name: str) -> tuple[str, str]:
        ...
        subject = issuer = x509.Name([
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, u"Secur-Serv"),
    ...
    def _create_pfx(self, cert_pem: str, key_pem: str, password: str) -> bytes:
        ...
        try:
            cmd = ['openssl', 'pkcs12', '-export', '-out', pfx_file_path, '-inkey', key_file_path, '-in', cert_file_path, '-password', f'pass:{password}']
    ...
    async def upload_certificate_to_ftp(self, cert_name: str):
        ...
        try:
            ...
            ftp_url = f"ftp://{settings.FTP_USER}:{settings.FTP_PASS}@{settings.FTP_HOST}/{filename}"
    ...
    async def import_certificate(self, session, cert_name: str, ftp_url: str, pfx_password: str) -> bool:
                ...
                if isinstance(status_info, dict):
                    ...
                    for info in info_list:
                        if info.get("level") == "error":
                            logger.error(f"   ❌ Import command reported error: {info.get('message', 'Unknown error')}")
                            return False

                    if status_info.get("success") is True:
                        for info in info_list:
                            if "successfully loaded certificate and key pair" in info.get("message", "").lower():
                                return True
        ...
        if status in [200, 201, 204]:
            if isinstance(response, dict) and 'status' in response:
                ...
                if isinstance(status_info, dict):
                    ...
                    for info in info_list:
                        if info.get("level") == "error":
                            if "not found" in info.get('message', '').lower() or "does not exist" in info.get('message', '').lower():
                                return True # Cert is already gone, so this is a success.
                            return False 
                        if "certificate has been successfully deleted" in info.get("message", "").lower():
                            return True
            return True
        else:
            return False
    ...
    async def run_complete_test(self):
        ...
        yield f"🚀 SonicWall Certificate Manager Test"
        ...
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=ssl_context)) as session:
            try:
                ...
                if await self.check_certificate_exists(session, self.current_test_cert_name, "initial_check"):
                    ...
                    if await self.check_certificate_exists(session, self.current_test_cert_name, "expecting_absent"):
                ...
                ftp_url, pfx_password = await self.upload_certificate_to_ftp(self.current_test_cert_name)
                if not ftp_url:
                ...
                if not await self.import_certificate(session, self.current_test_cert_name, ftp_url, pfx_password):
                ...
                if not await self.check_certificate_exists(session, self.current_test_cert_name, "expecting_absent"):
