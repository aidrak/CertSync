...
class SonicWallDeployManager:
    ...
    def __init__(self, hostname: str, username: str, password: str, management_port: int = 443,
    ...
    def _create_pfx(self, cert_pem: str, key_pem: str, password: str) -> bytes:
        ...
        cert_file_path, key_file_path, chain_file_path, pfx_file_path = None, None, None, None
        try:
    ...
    async def upload_certificate_to_ftp(self, cert_data: CertificateData, pfx_password: str) -> str:
        ...
        def _upload():
            try:
                ...
                if ftp_path:
                    ftp_url = f"ftp://{self.ftp_config['user']}:{urllib.parse.quote(self.ftp_config['pass'])}@{self.ftp_config['host']}/{ftp_path}/{filename}"
                else:
                    ftp_url = f"ftp://{self.ftp_config['user']}:{urllib.parse.quote(self.ftp_config['pass'])}@{self.ftp_config['host']}/{filename}"
    ...
    async def check_certificate_exists(self, session, cert_name: str) -> bool:
        ...
        status, response_data = await self._make_request(session, "POST", "/direct/cli", data=cli_command)
        ...
        if isinstance(response_data, dict) and 'status' in response_data:
            ...
            if isinstance(status_info, dict):
                ...
                for item in info_list:
                    ...
                    if item.get('level') == 'error' and ("no certificate found for this name" in message or "not a reasonable value" in message):
                        return False
...
exit"""
        ...
        status, response = await self._make_request(session, "POST", "/direct/cli", data=delete_commands)
                    ...
                    for info in info_list:
                        ...
                        if info.get("level") == "error":
                            if "not found" in message or "does not exist" in message:
                                logger.info(f"Certificate '{cert_name}' does not exist - ready for import")
                                return True
...
"""
        ...
        if status and status in [200, 201, 204]:
            ...
            if isinstance(response, dict):
                ...
                for info in info_list:
                    message = info.get("message", "").lower()
                    if any(err_msg in message for err_msg in ["loaded before", "already exists", "has been loaded before"]):
                        logger.info(f"Certificate '{cert_name}' already exists on SonicWall - proceeding with configuration")
                        return True
...
"""
        ...
        status, response = await self._make_request(session, "POST", "/direct/cli", data=ssl_vpn_config)
    ...
    async def commit_changes(self, session) -> bool:
        ...
        if status and status in [200, 201, 204]:
            if isinstance(response, dict) and response.get("status", {}).get("success") is not False:
    ...
    async def verify_ssl_vpn_configuration(self, session, cert_name: str) -> bool:
        """Verify that the SSL VPN is using the correct certificate using proven method from sonic-vpn-2.py"""
        ...
        if status in [200, 201, 204]:
            ...
            # Check if our certificate name appears in the response (exact match logic from sonic-vpn-2.py)
    ...
    async def check_certificate_exists_by_name(self, session, cert_name: str) -> bool:
        ...
        try:
            ...
            if status in [200, 201, 204]:
                ...
                logger.warning(f"Unclear response when checking certificate '{cert_name}': {response}")
                return False
        except Exception as e:
    ...
    async def cleanup_old_ssl_vpn_certificates(self, session, current_cert_name: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Found {len(existing_old_certs)} old SSL-VPN certificates to delete: {existing_old_certs}")
...
exit"""
                ...
                delete_status, delete_response = await self._make_request(session, "POST", "/direct/cli", data=delete_commands)
                
                if delete_status in [200, 201, 204]:
                    # Check if deletion was successful by looking at response
                    success_indicators = ["successfully deleted", "success", "not found", "does not exist"]
                    response_text = json.dumps(delete_response) if isinstance(delete_response, dict) else str(delete_response)
                    ...
                    if any(indicator in response_text.lower() for indicator in success_indicators):
                        ...
                        logger.warning(f"⚠️ Delete command sent for '{old_cert}' but success unclear")
                        cleanup_count += 1  # Count it as success anyway
                    
                    # Wait for deletion to process
                    await asyncio.sleep(1)
                else:
                    logger.error(f"❌ Failed to delete certificate '{old_cert}': HTTP {delete_status}")
            ...
            if cleanup_count > 0:
                ...
                if verified_deletions > 0:
                    logger.info(f"✅ Verified {verified_deletions}/{verification_count} deletions successful")
                    return True
                else:
                    logger.warning("⚠️ Could not verify any deletions - certificates may still exist")
                    return False
            else:
        ...
        try:
            async with aiohttp.ClientSession(connector=connector) as session:
                ...
                if "STAGING" in cert_data.cert_body or "staging" in cert_data.cert_body.lower():
                    ...
                    yield "  - ⚠️ For production use, please use a production Let's Encrypt certificate"
                ...
                if chain_count > 1:
                    ...
                    yield "  - ⚠️ WARNING: No certificate chain detected - this may cause verification issues"
                ...
                import_success = await self.import_certificate_via_cli(session, cert_name_for_sonicwall, ftp_url, pfx_password)
                ...
                ssl_config_success = await self.configure_ssl_vpn_certificate(session, cert_name_for_sonicwall)
                ...
                verification_success = await self.verify_ssl_vpn_configuration(session, cert_name_for_sonicwall)
                
                if not verification_success:
                    yield f"  - ❌ VERIFICATION FAILED - Certificate '{cert_name_for_sonicwall}' not active!"
                ...
                yield f"  - 🛡️ SAFETY: Will keep current certificate '{cert_name_for_sonicwall}' active"
                cleanup_success = await self.cleanup_old_ssl_vpn_certificates(session, cert_name_for_sonicwall)
                if cleanup_success:
                ...
                yield f"  - 🌐 SSL VPN is accessible with the new certificate"
                yield f"  - 🧹 Old certificates have been cleaned up"
                yield f"  - 📋 Deployment used timestamp naming to avoid conflicts"
