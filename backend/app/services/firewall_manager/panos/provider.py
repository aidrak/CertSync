...
class PanosManager(FirewallBase):
    ...
    async def import_certificate(self, cert_data: CertificateData) -> bool:
        ...
        try:
            ...
            data.add_field('file', keypair_content, filename=f"{cert_data.cert_name}.pem", content_type='application/x-pem-file')
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                async with session.post(self.base_url, params=params, data=data) as response:
                    ...
                    if response.status >= 400:
                        logger.error(f"Failed to import certificate: {response.status} - {response_text}")
                        return False
    ...
    async def apply_certificate(self, cert_name: str, service: str) -> bool:
        ...
        try:
            if service.lower() != "ssl-tls-service-profile":
                logger.error(f"Unsupported service for PAN-OS: {service}. Only 'ssl-tls-service-profile' is supported.")
                return False

            xpath = "/config/devices/entry[@name='localhost.localdomain']/vsys/entry[@name='vsys1']/ssl-tls-service-profile/entry[@name='default']"
            ...
            if root.attrib.get('status') == 'success':
                ...
                logger.error(f"Failed to apply certificate: {ET.tostring(root, encoding='unicode')}")
                return False
    ...
    async def test_connection(self):
        ...
        try:
            ...
            yield f"❌ Connection failed: Could not connect to {self.hostname}. Please check the IP/hostname and port."
