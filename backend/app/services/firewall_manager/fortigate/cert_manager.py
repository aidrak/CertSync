...
class FortiGateCertManager:
    ...
    async def _make_request(self, session, method: str, endpoint: str, data=None, timeout: int = 30):
        ...
        try:
            if method.upper() == "POST":
                response_obj = await session.post(url, headers=self.headers, json=data, timeout=timeout)
    ...
    async def check_certificate_exists(self, session, cert_name: str) -> bool:
        ...
        status, response = await self._make_request(session, "GET", f"/cmdb/vpn.certificate/local/{cert_name}")
        return status == 200

    async def delete_certificate(self, session, cert_name: str) -> bool:
        """Delete certificate."""
        logger.info(f"🗑️ Deleting certificate '{cert_name}'...")
        status, response = await self._make_request(session, "DELETE", f"/cmdb/vpn.certificate/local/{cert_name}")
        return status == 200
