...
class FortiGateManager(FirewallBase):
    def __init__(
        ...
        self.cert_manager = FortiGateCertManager(
            hostname=hostname, api_key=api_key, verify_ssl=verify_ssl
        )
        ...
        try:
            async with aiohttp.ClientSession(connector=connector) as session:
                ...
                if await self.cert_manager.check_certificate_exists(
                    ...
                    status, result = await self.cert_manager._make_request(
                        session,
                        "PUT",
                        f"/cmdb/vpn.certificate/local/{cert_data.cert_name}",
                        {"data": payload},
                    )
                else:
                    logger.info(
                        f"Importing new certificate {cert_data.cert_name} to {self.hostname}"
                    )
                    status, result = await self.cert_manager._make_request(
                        session,
                        "POST",
    ...
    async def test_connection(self):
        ...
        """
        logger.info(
            "test_connection on manager for %s called,"
            " but validation is handled by the dedicated validator.",
            self.hostname,
        )
