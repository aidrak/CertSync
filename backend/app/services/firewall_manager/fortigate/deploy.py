...
class FortiGateDeployManager:
    ...
    async def _make_request(self, session, method: str, endpoint: str, data: dict = None):
    ...
    async def get_current_vpn_settings(self, session) -> tuple[bool, str, dict]:
        ...
        if status == 200:
            ...
            logger.info(f"Current SSL VPN settings - Status: {vpn_status}, Certificate: {current_cert}")
        ...
        try:
            async with aiohttp.ClientSession(connector=connector) as session:
                ...
                if not success:
                    ...
                    yield f"💡 Check API key, FortiGate connectivity, and admin privileges"
                    return
                ...
                payload = {
                    "servercert": cert_data.cert_name
                }
                
                status, result = await self._make_request(session, "PUT", "/cmdb/vpn.ssl/settings", payload)
                ...
                if success and current_cert == cert_data.cert_name:
                    yield f"✅ VPN certificate assignment verified!"
                    yield f"🎉 SSL VPN CERTIFICATE DEPLOYMENT SUCCESSFUL!"
                    yield f"✅ Certificate '{cert_data.cert_name}' is now the active SSL VPN certificate"
                    yield f"🌐 SSL VPN URL: https://{self.hostname}:{self.management_port}"
                else:
                    yield f"❌ VPN certificate assignment verification failed"
    ...
    async def restore_original_vpn_certificate(self, original_cert: str, test_cert: str) -> AsyncIterator[str]:
        ...
        if original_cert and original_cert != test_cert:
            ...
            try:
                async with aiohttp.ClientSession(connector=connector) as session:
                    ...
                    payload = {
                        "servercert": original_cert
                    }
                    
                    status, result = await self._make_request(session, "PUT", "/cmdb/vpn.ssl/settings", payload)
                    
                    if status == 200:
                        yield f"✅ Original VPN certificate restored"
                    else:
            ...
            yield f"ℹ️ No original certificate to restore"