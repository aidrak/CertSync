...
class FortiGateValidator:
    ...
    def _generate_test_certificate(self):
        ...
        subject = issuer = x509.Name(
            [
                ...
                x509.NameAttribute(NameOID.ORGANIZATION_NAME, "CertSync-Test"),
                x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Validation"),
                x509.NameAttribute(
                    NameOID.COMMON_NAME, self.test_cert_name
                ),  # This already uses self.test_cert_name
                x509.NameAttribute(NameOID.EMAIL_ADDRESS, "admin@certsync.test"),
            ]
        )
    ...
    async def _import_cert(self, session, cert_name) -> tuple[bool, str]:
        ...
        if status == 200:
            ...
            return (
                False,
                (
                    "❌ Certificate format invalid (error -327). FortiGate rejected the "
                    "certificate format. This may indicate an issue with certificate "
                    "encoding or API compatibility."
                ),
            )
            ...
            return (
                False,
                (
                    "❌ Certificate validation failed (error -328). The certificate "
                    "content may be invalid or incompatible with this FortiGate version."
                ),
            )
        elif status == 424:
            return (
                False,
                (
                    "❌ Missing required parameters (HTTP 424). The API request is missing "
                    "mandatory fields."
                ),
            )
        else:
    ...
    async def _test_certificate_update(self, session, cert_name) -> tuple[bool, str]:
        ...
        status, result = await self._make_request(
            session, "PUT", f"/cmdb/vpn.certificate/local/{cert_name}", payload
        )
    ...
    async def run_complete_test(self):
        yield "🚀 Starting FortiGate Comprehensive Validation..."
        ...
        try:
            async with aiohttp.ClientSession(connector=connector) as session:
                ...
                if exists:
                    yield "🗑️ Attempting to delete pre-existing certificate..."
                ...
                yield "🔧 Generating fresh test certificate..."
                yield "➕ Attempting to import test certificate..."
                ...
                # 5. Final Cleanup
                yield "🗑️ Cleaning up test certificate..."
                cleaned, msg = await self._delete_cert(session, self.test_cert_name)
                yield msg
                if not cleaned:
                    yield (
                        f"⚠️ Failed to clean up test certificate '{self.test_cert_name}'. "
            ...
            yield (
                f"❌ Connection Failed: Could not connect to the FortiGate at "
                f"{self.hostname}. Error: {e}"
            )
