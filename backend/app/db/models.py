from sqlalchemy import Column, Integer, String, DateTime, ForeignKey, Text, Enum, Table, UniqueConstraint, Boolean
...
class DnsProviderAccount(Base):
    ...
    __table_args__ = (UniqueConstraint('managed_domain', 'company', name='dns_provider_managed_domain_company_unique'),)
...
class TargetSystem(Base):
    ...
    __table_args__ = (UniqueConstraint('system_name', 'system_type', name='target_systems_system_name_type_unique'),)
...
class Certificate(Base):
    ...
    dns_provider_account_id = Column(Integer, ForeignKey("dns_provider_accounts.id"), nullable=False)
