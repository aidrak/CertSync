...
def get_target_system_by_name(db: Session, system_name: str, system_type: str):
    return db.query(models.TargetSystem).filter(
...
def create_target_system(db: Session, target_system: schemas.TargetSystemCreate):
    ...
    encrypted_admin_password = encrypt_secret(target_system.admin_password) if target_system.admin_password else None
...
def update_target_system(db: Session, target_system_id: int, target_system: schemas.TargetSystemUpdate):
