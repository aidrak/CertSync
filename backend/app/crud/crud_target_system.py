...
def get_target_system(db: Session, target_system_id: int):
    return (
        ...
        .filter(models.TargetSystem.id == target_system_id)
        .first()
    )
...
def create_target_system(db: Session, target_system: schemas.TargetSystemCreate):
    encrypted_api_key = (
        encrypt_secret(target_system.api_key) if target_system.api_key else None
    )
    encrypted_admin_password = (
        encrypt_secret(target_system.admin_password)
        if target_system.admin_password
        else None
    )
...
def update_target_system(
    ...
    if db_target_system:
        ...
        if "admin_password" in update_data and update_data["admin_password"]:
            update_data["admin_password"] = encrypt_secret(
