...
def create_dns_provider_account(db: Session, account: dns_schemas.DnsProviderAccountCreate):
    try:
        ...
        test_parsed = json.loads(test_decrypted)
...
def get_dns_provider_account(db: Session, account_id: int):
    return db.query(models.DnsProviderAccount).filter(models.DnsProviderAccount.id == account_id).first()

from typing import Optional

def get_dns_provider_account_by_domain(db: Session, managed_domain: str, company: str):
    return db.query(models.DnsProviderAccount).filter(
...
def update_dns_provider_account(db: Session, account_id: int, account: dns_schemas.DnsProviderAccountUpdate):
...
def delete_dns_provider_account(db: Session, account_id: int):
    try:
        db_account = db.query(models.DnsProviderAccount).filter(models.DnsProviderAccount.id == account_id).first()
        if db_account:
            # Check if the DNS provider is associated with any certificates
            associated_certificates = db.query(models.Certificate).filter(models.Certificate.dns_provider_account_id == account_id).count()
            if associated_certificates > 0:
                raise HTTPException(
                    status_code=400,
                    detail=f"Cannot delete DNS provider. It is currently associated with {associated_certificates} certificate(s). Please reassign them before deleting."
                )
