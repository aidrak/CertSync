...
def create_dns_provider_account(
    ...
    try:
        ...
        existing_account = crud_dns.get_dns_provider_account_by_domain(
            db, managed_domain=account.managed_domain, company=account.company
        )
        if existing_account:
            raise HTTPException(
                status_code=409,
                detail="An account with this name and company already exists.",
            )
...
@router.put(
    "/dns-provider-accounts/{account_id}",
    response_model=dns_schemas_dns.DnsProviderAccount,
)
def update_dns_provider_account(
    account_id: int,
    account: dns_schemas_dns.DnsProviderAccountUpdate,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(require_role("technician")),
):
    try:
        db_account = crud_dns.update_dns_provider_account(
...
def delete_dns_provider_account(
    account_id: int,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(require_role("technician")),
):
...
def test_dns_provider_account(
    ...
    try:
        ...
        if not validator.run_all_tests():
            raise HTTPException(
                status_code=400,
                detail=f"{account.provider_type.value.capitalize()} token test failed.",
            )
        return account
    except Exception as e:
