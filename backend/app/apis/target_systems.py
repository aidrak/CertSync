...
from ..core.exceptions import handle_sse_exception, handle_generic_exception, NotFoundError, ConflictError
from fastapi.responses import StreamingResponse
from sqlalchemy.orm import Session
from ..crud import crud_target_system, crud_log
from ..schemas import schemas
from ..db.database import get_db
from ..dependencies import require_role, require_admin_or_technician, get_current_user
from ..db.models import UserRole, User, TargetSystemType
...
def create_target_system(target_system: schemas.TargetSystemCreate, db: Session = Depends(get_db), current_user: User = Depends(require_role("technician"))):
    ...
    logger.info(f"Target system '{new_target_system.system_name}' created successfully by user '{current_user.username}'.")
    ...
    crud_log.create_log(
        ...
        log=schemas.LogCreate(
            ...
            message=f"User '{current_user.username}' created a new target system: {new_target_system.system_name}",
        ),
...
def read_target_systems(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: User = Depends(require_admin_or_technician)):
...
def read_target_system(target_system_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_admin_or_technician)):
...
def update_target_system(target_system_id: int, target_system: schemas.TargetSystemUpdate, db: Session = Depends(get_db), current_user: User = Depends(require_role("technician"))):
    ...
    logger.info(f"Target system '{db_target_system.system_name}' updated successfully by user '{current_user.username}'.")
    crud_log.create_log(
        db=db,
        log=schemas.LogCreate(
            level="info",
            ...
            message=f"User '{current_user.username}' updated target system: {db_target_system.system_name}",
        ),
        user_id=getattr(current_user, 'id', None)
    )
...
def delete_target_system(target_system_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_role("technician"))):
    ...
    logger.info(f"Target system '{db_target_system.system_name}' deleted successfully by user '{current_user.username}'.")
    crud_log.create_log(
        db=db,
        log=schemas.LogCreate(
            level="info",
            ...
            target=f"Target System ID: {db_target_system.id}",
            message=f"User '{current_user.username}' deleted a target system: {db_target_system.system_name}",
        ),
        user_id=getattr(current_user, 'id', None)
    )
