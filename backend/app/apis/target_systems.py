...
from ..core.exceptions import handle_sse_exception, NotFoundError, ConflictError
...
from ..db.models import User, TargetSystemType
...
def create_target_system(
    ...
    logger.debug(
        f"Attempting to create target_system with name: {target_system.system_name}"
    )
    ...
    if db_target_system:
        logger.warning(
            f"Target system with name {target_system.system_name} already exists."
        )
        raise ConflictError("Target system name already registered")

    new_target_system = crud_target_system.create_target_system(
        db=db, target_system=target_system
    )
    logger.info(
...
def read_target_system(
    ...
    db_target_system = crud_target_system.get_target_system(
...
def update_target_system(
    ...
    if db_target_system is None:
        logger.warning(
            f"Target system with ID {target_system_id} not found for update."
        )
        raise NotFoundError("Target system")
...
def delete_target_system(
    ...
    if db_target_system is None:
        logger.warning(
            f"Target system with ID {target_system_id} not found for deletion."
        )
        raise NotFoundError("Target system")
...
async def test_connection_sse(
    ...
    async def event_generator():
        ...
        try:
            ...
            if not password:
                raise ValueError(
                    "Missing credentials. Provide either api_key or admin_password."
                )
