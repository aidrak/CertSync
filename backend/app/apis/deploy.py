...
def read_deployment(
    ...
    return {
        ...
        "certificate": {
            ...
            if db_deployment.certificate.dns_provider_account
            else None,
    ...
    try:
        ...
        if deployment_type == "ssl_vpn":
            ...
            if success:
                ...
                return {
                    "message": "SSL VPN certificate deployment completed successfully",
                ...
                return {
                    "message": "SSL VPN certificate deployment failed",
                    "logs": deployment_logs,
                }
        else:
            ...
            if success:
                crud_deployment.update_deployment_status(
                    db, deployment_id=deployment_id, status=DeploymentStatus.success
                )
...
async def verify_vpn_deployment(
    ...
    try:
        ...
        success = any(
            "successful" in log.lower() or "verified" in log.lower()
            for log in verification_logs[-3:]
        )
    ...
    try:
        ...
        if target_system.system_type.value == "sonicwall":
            ...
            if result["success"]:
                return {
                    "message": (
                        f"Certificate renewal and deployment completed "
                        f"successfully for {certificate.common_name}"
                    ),
                    "deployment_id": deployment_id,
            ...
            renewal_result = await auto_renewal_service.renew_certificate_cloudflare(
                db, certificate
            )
            ...
            if renewal_result["success"]:
                return {
                    "message": (
                        f"Certificate renewed successfully for {certificate.common_name}. "
                        f"Manual deployment required for {target_system.system_type.value} systems."
                    ),
                    "deployment_id": deployment_id,
                    "certificate_name": certificate.common_name,
                    "target_system": target_system.system_name,
                    ...
                    "note": (
                        ...
                        f"{target_system.system_type.value}. Please deploy manually."
                    ),
                }
            else:
                raise HTTPException(
                    ...
                    detail=(
                        ...
                        f"{renewal_result.get('error', 'Unknown error')}"
                    ),
                )
...
@router.get(
    "/certificates-by-company/{company}",
...
def get_certificates_by_company(
    ...
    certificates = (
        ...
        .join(DnsProviderAccount)
        .filter(DnsProviderAccount.company == company)
        .all()
    )
    return certificates
...
@router.get(
    "/target-systems-by-company/{company}",
    response_model=List[schemas.TargetSystem],
)
